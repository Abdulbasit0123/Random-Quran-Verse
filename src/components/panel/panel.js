...
import { isOnAutoplay, playModeState } from '../../events/actions.js';
...
function autoplayAudio(){//I think you should change that, but for now it does its job.
    pauseAudio(); 
    if(isOnAutoplay){
        playAudio();
    }
...
function storePanel(surahIndex, ayahIndex, ayahNumber) {
...
export function updateCurrentState() {
    ...
    currentAyahNumber = data.ayahNumber;
}
...
export function getGlobalAyahNumber(surahIndex, ayahIndex) {
    let ayahNumber = surahLength
        .slice(0, surahIndex)      // get all surahs before the current one
        .reduce((sum, len) => sum + len, 0)  // sum them
        + (ayahIndex + 1);          // add current ayah (1-based)
    return ayahNumber;
}

export async function createPanel(surahIndex, ayahIndex) {
    storePanel(surahIndex, ayahIndex, getGlobalAyahNumber(surahIndex, ayahIndex));
